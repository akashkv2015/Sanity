...
Add STO Product to Cart and Verify
    ...
    [Documentation]     Verify STO product is being added to cart successfully
    sleep       3s
    click on element   xpath:(//a[@href="javascript:void(0);"])[2]
    Sleep   4s
    ${productName} =    Get Text    ${stoProductName}
#    click on element    ${elem}
    click on element   xpath:(//a[text()='Add to cart'])[1]
    sleep       3s
    run keyword if      '${tech}'=='WEB'    run keywords
    ...     Mouse Over      ${miniCartIcon}
    ...     AND     Verify Element Exists       ${miniCartTitle}
...
Add STO Product to Cart and Verify in Header
    [Arguments]     ${elem}     ${stoProductName}
    [Documentation]     Verify STO product is being added to cart successfully
    sleep       3s
    ${productName} =    Get Text    ${stoProductName}
...
#    wait until element is visible   ${AltImg}       ${Timeout}    "${AltImg}" is not visible on the screen
    Run Keyword And Ignore Error    Click Element  ${CarouselIcon}
    Run Keyword And Ignore Error    ${AltImg}  "Image and alternate images is not displayed when we click on carousel icon"
...
Go to Top sellers section and add STO product
    ...
    Add STO Product to Cart and Verify in Header    ${locator_addtocart_btn}      ${locator_stoproduct_title}
...
Verify & add new shipping address
    ...
    wait until element is visible   ${enter_no}
    Enter Value         ${enter_no}   7653458769627282
...
#    Click On Element    //a[@class='makedefaultlink']
    Click On Element    //a[contains(text(),'Make default')]
